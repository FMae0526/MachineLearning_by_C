...
/*GAによって、ナップサック問題の最良解を探索	*/
/*使い方					*/
/*/home/ML02/GA/kpga,c 実行時:./kpga		*/
/*						*/
/************************************************/
...
#define POOLSIZE 30			/* プール制限 */
#define LASTG 50			/* 打ち切り制限 */
#define MRATE 0.01			/* 突然変異の確率 */
#define SEED 327627			/* 乱数のシード */
...
int selectp(int roulette[POOLSIZE], int totalfitness);
						/* 親の選択 */
void crossing(int m[], int p[], int cl[], int c2[]);
						/* 特定の2染色体の交叉 */
...
int elite, bestfit = 0;		/* エリート遺伝子の処理変数 */
...
while((rndno = ((double)rand() / RAND_MAX) * 1) == 1);
