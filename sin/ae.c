...
#define HIDDENNO 3			/* 中間層のセル層 */
#define OUTPUTNO 9			/* 出力層の人工ニューロン数 */
...
double  forward(double wh[HIDDENNO][INPUTNO + 1],
				...
				double e[]);	/* 順方向の計算 */
void olearn(double wo[HIDDENNO], double hi[],
...
double e[MAXINPUTNO][INPUTNO + OUTPUTNO];	/* データセット */
...
double dj;			/* 中間層の重み計算に利用 */
   ...
   {		/* 中間層の各セルjを対象 */
	dj = hi[j] * (1 - hi[j]) * wo[j] * (e[INPUTNO] + k) * o * (1 - o);
